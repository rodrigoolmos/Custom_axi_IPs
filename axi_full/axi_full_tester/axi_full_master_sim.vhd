...
entity axi_full_master_sim is
	...
	port (
		...
		BASE_AWADDR 	: in std_logic_vector(C_M_AXI_ADDR_WIDTH-1 downto 0);
		BASE_ARADDR		: in std_logic_vector(C_M_AXI_ADDR_WIDTH-1 downto 0);
		N_BURSTs_RW		: in std_logic_vector(C_M_AXI_ADDR_WIDTH-1 downto 0);
		M_AXI_WDATA_TB	: in std_logic_vector(C_M_AXI_ADDR_WIDTH-1 downto 0);
		M_AXI_RDATA_TB	: out std_logic_vector(C_M_AXI_ADDR_WIDTH-1 downto 0);
		M_AXI_WSTRB_TB	: in std_logic_vector(C_M_AXI_DATA_WIDTH/8-1 downto 0);
...
end axi_full_master_sim;

architecture implementation of axi_full_master_sim is
	...
	-- Non-2^n lengths will eventually cause bursts across 4K address boundaries.
	 signal  C_MASTER_LENGTH  : integer := 0;
	-- total number of burst transfers is master length divided by burst length and burst size
     constant  AUX : integer := clogb2((C_M_AXI_BURST_LEN*C_M_AXI_DATA_WIDTH/8)-1);

	 signal  C_NO_BURSTS_REQ  : integer := 0;
...
begin
	...
	C_MASTER_LENGTH <= to_integer(signed(N_BURSTs_RW));
	C_NO_BURSTS_REQ  <= (C_MASTER_LENGTH-AUX);
	READ_VALID <= axi_rready and M_AXI_RVALID;
	WRITE_VALID <= M_AXI_WREADY and axi_wvalid;
	M_AXI_WDATA	<= M_AXI_WDATA_TB;
	M_AXI_RDATA_TB <= M_AXI_RDATA;
	--I/O Connections. Write Address (AW)
	M_AXI_AWID	<= (others => '0');
	--The AXI address is a concatenation of the target base address + active offset range
	M_AXI_AWADDR	<= std_logic_vector( unsigned(BASE_AWADDR) + unsigned(axi_awaddr) );
	...
	--Read Address (AR)
	M_AXI_ARID	<= (others => '0');
	M_AXI_ARADDR	<= std_logic_vector( unsigned(BASE_ARADDR) + unsigned( axi_araddr ) );
	--Burst LENgth is number of transaction beats, minus 1
	...
	--Example design I/O
	SYSTEM_IDLE	<= '0' when mst_exec_state /= IDLE or INIT_AXI_TXN = '1'
						or init_txn_ff2 = '1'
						or init_txn_ff = '1'
						or INIT_AXI_RXN = '1'
						or init_rxn_ff2 = '1'
	...
	--Generate a pulse to initiate AXI TX transaction.
	process(M_AXI_ACLK)                                                          
	begin                                                                             
	...
	--Generate a pulse to initiate AXI RX transaction.
		process(M_AXI_ACLK)                                                          
	begin                                                                             
	  if (rising_edge (M_AXI_ACLK)) then                                              
	      -- Initiates AXI transaction delay        
	  ...
	  begin                                                                                                      
	    if (rising_edge (M_AXI_ACLK)) then                                                                       
	      if (M_AXI_ARESETN = '0' or init_txn_pulse = '1') then                                                                         
	        ...
	        if (M_AXI_AWREADY = '1' and axi_awvalid = '1') then                                                  
	          if (unsigned(write_burst_counter) < C_NO_BURSTS_REQ)then                                                
	  ...
	  begin                                                                                                      
	    if (rising_edge (M_AXI_ACLK)) then                                                                       
	      if (M_AXI_ARESETN = '0' or init_rxn_pulse = '1') then                                                                         
	        ...
	        if (M_AXI_ARREADY = '1' and axi_arvalid = '1') then                                                  
	          if (unsigned(read_burst_counter) < C_NO_BURSTS_REQ)then                                                 
	  ...
	  begin                                                                                                      
	    if (rising_edge (M_AXI_ACLK)) then                                                                       
	      ...
	      --elsif (M_AXI_RVALID && axi_rready && (read_burst_counter == {(C_NO_BURSTS_REQ-1){1}}) && axi_rlast)  
	        ...
	        if (M_AXI_BVALID = '1' and (unsigned(write_burst_counter) >= C_NO_BURSTS_REQ) and axi_bready = '1') then   
	          writes_done <= '1';                                                                                
	        end if;                                                                                              
	      end if;                                                                                                
	    end if;                                                                                                  
	  ...
	  begin                                                                                                      
	    if (rising_edge (M_AXI_ACLK)) then                                                                       
	      if (M_AXI_ARESETN = '0' or init_rxn_pulse = '1') then                                                                         
	        ...
	        if (M_AXI_RVALID = '1' and axi_rready = '1' and (read_index = std_logic_vector (to_unsigned(C_M_AXI_BURST_LEN-1,C_TRANSACTIONS_NUM+1))) and (unsigned(read_burst_counter) >= C_NO_BURSTS_REQ)) then
	          reads_done <= '1';                                                                                 
	        end if;                                                                                              
	      end if;                                                                                                
	    end if;                                                                                                  
